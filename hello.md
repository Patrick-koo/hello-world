...
- 특정 파일의 내용을 마지막 커밋으로 돌리고 싶으면 해당 파일 선택 후 `코드 뭉치 버리기` 선택
...
- 소스트리의 체크아웃 : 브랜치 이름을 더블 클릭하는 것 만으로 체크아웃 가는하다.
...
- 역시 커밋은 없어지지 않는다.
- 장점 : 가장 정석적, 쉽게 커밋을 되돌릴 수 있다.
-       revert로 되돌리면 이전 커밋이 남기 때문에 좋다.
- 단점 : 충돌이 일어날 수 있다.
- 사용 방법 : SourceTree에서는 커밋 선택 - 우클릭 - 커밋 되돌리기
...
- 단점 : 잘못하면 위험하다. 이미 원격 저장소에 올라간 경우 + 협업을 하고
-        있는 경우 특히 위험하다.
